...
const Particles = ({
  ...
  const drawStar = (circle, update = false) => {
    if (context.current) {
      ...
      context.current.moveTo(0, -outerRadius);

      for (let i = 1; i < starPoints * 2; i++) {
        const radius = i % 2 === 0 ? outerRadius : innerRadius;
        const angle = (Math.PI * i) / starPoints;
        context.current.lineTo(Math.sin(angle) * radius, -Math.cos(angle) * radius);
      }
